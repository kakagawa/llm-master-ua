---
title: Agent系统如何约束大模型幻觉：Prompt、工具调用、RAG证据、输出校验与兜底处理
description: 深度拆解Agent系统如何约束大模型幻觉：Agent幻觉类型（知识幻觉、工具幻觉、参数幻觉、证据幻觉、行动幻觉），四层防线（Prompt约束、工具调用约束、RAG证据约束、输出校验），以及约束后仍然出现幻觉时的拦截、降级、转人工、trace复盘、eval治理和监控指标，附大厂面试回答框架。
keywords: [Agent幻觉, 大模型幻觉, Agent可靠性, 工具调用幻觉, RAG幻觉, 证据校验, 输出校验, LLM-as-Judge, Agent兜底, Agent面试题, 大模型面试题, 大模型应用开发, AI求职]
...
1. 先搞清楚：Agent里的幻觉不止一种
2. 总体思路：不要指望一层防线解决所有问题
3. 第一层：Prompt约束，只能管边界，管不住全部
4. 第二层：工具调用约束，防止Agent乱行动
5. 第三层：证据约束，让回答有据可依
6. 第四层：输出校验，生成完不是结束
7. 约束之后还幻觉，怎么办？
8. 工程上怎么长期治理幻觉
...
1. **Prompt约束**：告诉模型什么能做、什么不能做
2. **工具约束**：限制它能调什么工具、参数能怎么填
3. **证据约束**：要求回答必须有来源，没有证据就拒答
4. **输出校验**：生成后再检查，不能让结果直接裸奔给用户
...
之前在 [RAG落地最难的地方在哪](./rag_hardest_parts_interview.md) 里讲过，RAG 最难的不是某一个环节，而是文档预处理、召回质量、生成忠实度三个环节级联放大。
