---
title: 大模型微调面试详解：SFT、RLHF、DPO、PPO、强化学习和基模变强后的优化价值
description: 系统拆解大模型微调面试高频问题：SFT监督微调、RLHF人类反馈强化学习、RL强化学习、PPO、DPO分别是什么，和Prompt、RAG怎么选，基模能力越来越强后垂类微调是否还值得做，SFT/RLHF/RL的破局点在哪里，以及如何从数据、评测、成本、业务边界判断微调是否有价值。
keywords: [大模型微调, SFT, RLHF, DPO, PPO, 强化学习, 监督微调, 人类反馈强化学习, 偏好优化, 模型微调面试, 大模型面试题, LLM微调, 垂类模型微调, RAG和微调, Prompt和微调, AI求职]
tags: [大模型面试, LLM面试, 模型微调, AI求职, 大模型求职]
---

# 大模型微调面试怎么答？SFT、RLHF、DPO、PPO 到底还有没有必要？

最近[知识星球](https://programmercarl.com/other/kstar.html)有录友分享了一道很典型的大模型面试题：
...
1. 先说结论：微调没有消失，只是价值变了
2. 先把名词讲清楚：SFT、RLHF、RL、PPO、DPO 是什么
3. 微调、Prompt、RAG 到底怎么选
4. 为什么很多垂类微调会被基模抹平
5. SFT 的破局点：从补知识变成控行为
6. RLHF / RL 的破局点：从变聪明变成控偏好和控决策
7. DPO 为什么现在经常被提到
8. 什么场景不建议做微调
9. 工程上怎么判断微调有没有收益
10. 面试官可能怎么追问
...
之前写 [RAG落地最难的地方在哪](./rag_hardest_parts_interview.md) 时讲过，RAG 更适合处理外部知识和动态文档。
...
之前在 [Agent系统如何约束大模型幻觉](./agent_hallucination_control_interview.md) 里讲过，Agent 不能只靠 Prompt，工具调用和高风险动作必须有工程层拦截。
