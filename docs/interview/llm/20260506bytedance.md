---
title: 字节Agent开发四面面经：21道大模型面试题全解析，从Prompt到Agent到MCP
description: 字节跳动Agent开发岗1～4面面经分享，从面经中提炼21道经典面试题，覆盖Prompt工程、幻觉控制、Function Calling与RAG、Agent系统设计、MCP与Skill、微调与RLHF、上下文记忆管理、混合路由限流、传统Web vs Agent等核心考点，每题附面试场景回答方向。
keywords: ["字节跳动面经", "Agent开发面试", "大模型面试题", "Prompt Engineering", "Function Calling", "RAG", "MCP", "Skill", "Agent架构", "上下文窗口", "幻觉", "微调", "RLHF", "多Agent", "混合路由"]
tags: ["面经", "大模型", "Agent开发"]
---
...
这次四面覆盖的考点很全，每个方向卡码笔记都有对应的深度拆解，可以对照着补：[Agent 大厂面试题汇总](./agent_interview.md)（ReAct、Function Calling、MCP）、[RAG 大厂面试题汇总](./rag_interview.md)（检索、Rerank、幻觉）、[SFT、RLHF、DPO 面试详解](./finetuning_sft_rlhf_interview.md)（微调）、[Agent Skill 面试详解](./agent_skill_interview.md)（能力复用）、[Agent 混合路由优化详解](./agent_hybrid_routing_interview.md)（限流路由）、[Agent 系统如何约束大模型幻觉](./agent_hallucination_control_interview.md)（幻觉控制）。如果想看面试官围着一个项目连续追问指标、三层记忆、DAG 编排和 Token 治理，可以接着读[字节番茄小说 Agent 应用开发实习一面](./bytedance_fanqie_agent_intern_interview.md)。

下面是他在[知识星球](https://programmercarl.com/other/kstar.html)里分享的面经经历、心得，以及面试题。
