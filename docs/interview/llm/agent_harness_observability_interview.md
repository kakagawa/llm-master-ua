---
title: Agent Harness可观测性面试详解：生产级AI项目如何做好Trace、工具调用、上下文、成本与评测闭环
description: 深度拆解Agent Harness可观测性为什么是生产级AI项目必须补上的一课，覆盖Agent从Demo走向生产后的Trace轨迹记录、状态变化、工具调用归因、上下文污染检测、Token成本监控、线上失败复盘、评测回归和面试回答框架，适合准备Agent工程化、LLM应用开发和大模型求职的录友。
keywords: [Agent Harness可观测性, Agent Observability, Agent Trace, LLM Trace, Agent生产化, Agent工程化, Harness Engineering, AI可观测性, 大模型可观测性, Agent工具调用, Agent上下文管理, Agent成本监控, Agent评测, 大模型面试题, LLM面试, AI求职]
...
后面又聊过 [Multi-Agent Harness](./multi_agent_harness_interview.md)，讲的是多 Agent 真正进生产，拼的不是 Agent 数量，而是谁的执行框架更稳。
...
1. [为什么 Agent 能跑起来了，生产里还是不稳？](#一为什么-agent-能跑起来了生产里还是不稳)
2. [Agent Harness 可观测性到底是什么？](#二agent-harness-可观测性到底是什么)
3. [Agent 可观测性和普通日志有什么区别？](#三agent-可观测性和普通日志有什么区别)
4. [一个 Agent Trace 里到底要记录什么？](#四一个-agent-trace-里到底要记录什么)
5. [长链路任务里，怎么发现 Agent 已经跑偏了？](#五长链路任务里怎么发现-agent-已经跑偏了)
6. [工具调用出问题，怎么定位是模型错、参数错，还是工具错？](#六工具调用出问题怎么定位是模型错参数错还是工具错)
7. [Agent 成本失控，应该监控哪些指标？](#七agent-成本失控应该监控哪些指标)
8. [线上失败样本，怎么沉淀成 Harness 的长期改进？](#八线上失败样本怎么沉淀成-harness-的长期改进)
9. [如果让你设计一套生产级 Agent 可观测系统，你会怎么答？](#九如果让你设计一套生产级-agent-可观测系统你会怎么答)
...
这个问题和之前聊过的 [Agent上下文漂移](./agent_drift_hallucination_interview.md) 强相关。
