---
title: Spec-Driven Development规约驱动开发：AI编程为什么越快越要先写规格
description: 深入讲解Spec-Driven Development规约驱动开发的背景、核心流程和工程价值，分析AI编程中需求歧义、上下文漂移、返工、验收困难等问题，并结合规格、技术计划、任务拆分、测试验证和变更同步说明如何落地，适合AI编程、Agent工程化和大模型求职面试。
keywords: [Spec-Driven Development, SDD, 规约驱动开发, 规格驱动开发, 规范驱动开发, AI编程, AI辅助开发, Vibe Coding, Coding Agent, Agent工程化, GitHub Spec Kit, Kiro, OpenSpec, 需求工程, 软件规格说明书, PRD, TDD, BDD, 验收标准, 技术方案, 任务拆分, 上下文工程, AI编程面试, 大模型面试, LLM面试]
tags: [AI编程面经, 规约驱动开发, Agent工程化, 大模型面试, AI求职]
---

# Spec-Driven Development规约驱动开发：AI写代码越快，为什么越要先写清楚

前面我们聊过 [Vibe Coding](./vibe_coding_interview.md)，核心问题是：AI 可以把代码写得很快，但“写得快”不等于“写得对”。

后面又聊了 [AI 增强开发三件套](./ai_enhanced_development_openspec_superpowers_gstack.md)，其中 OpenSpec 负责把模糊需求变成可审查的规格。
...
- 未过期会员续费，到底从原到期日顺延，还是从付款日重算，AI 自己猜了一个
- 支付回调重复到达会不会重复增加时长，需求没写，AI 只实现了成功路径
- 你在前一轮对话里说过“不能改旧接口”，上下文压缩后它忘了
- 一个任务同时改了十几个文件，等你 Review 时，已经很难找到从哪一步开始跑偏
- 测试虽然全过，但测试验证的是 AI 自己写出的逻辑，不是你真正想要的业务规则
...
GitHub 在 2025 年 9 月发布开源 [Spec Kit](https://github.blog/ai-and-ml/generative-ai/spec-driven-development-with-ai-get-started-with-a-new-open-source-toolkit/) 时，把核心流程整理成 `Specify → Plan → Tasks → Implement`。[Kiro](https://kiro.dev/blog/introducing-kiro/) 也在 2025 年 7 月发布时，把 requirements、design、tasks 做成 Coding Agent 可以持续读取和执行的工件。
...
- 模糊词：“尽快”“友好”“支持常见格式”到底是什么意思
- 缺失场景：失败、超时、重复、并发、权限不足怎么办
- 冲突规则：两个需求同时触发时听谁的
- 隐藏约束：旧接口、数据迁移、合规、性能有没有限制
- 范围膨胀：哪些需求看似顺手，其实不在本次交付里
...
1. 为续费订单增加幂等键和唯一约束，并写重复请求测试。
2. 实现会员期限顺延规则，并覆盖未过期、刚过期和跨月场景。
3. 接入支付回调状态流转，并补失败补偿测试。
4. 增加续费页面和错误提示，完成端到端验证。
...
- 改了哪些文件，diff 是否符合当前任务
- 单元测试、集成测试、端到端测试分别通过了什么
- 有没有修改规约之外的代码
- 有没有遗留假设、失败测试或未完成项
- 当前实现能不能追溯到验收标准
...
这和 [Claude Code 上下文窗口](./claude_code_context_window_interview.md) 里讲的道理是一致的：上下文窗口会压缩、会丢细节，关键约束必须沉淀到外部工件，不能只藏在对话里。
...
现在常见的做法大致有三层。[GitHub Spec Kit 的规约持久化说明](https://github.github.com/spec-kit/concepts/spec-persistence.html)也强调，团队要先约定规约在功能完成后保留多久、需求变化时以哪份工件为准。
