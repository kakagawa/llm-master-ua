---
title: GraphRAG与LightRAG大厂面试题汇总：从RAG到知识图谱检索，传统RAG天花板、GraphRAG原理与坑、LightRAG轻量方案
description: GraphRAG与LightRAG方向高频面试题汇总，覆盖传统RAG在全局关系、多跳推理、社区摘要上的天花板，拆解GraphRAG索引和查询全流程、落地常见坑、LightRAG双层检索与增量插入机制，并给出GraphRAG与LightRAG选型和面试回答思路。
keywords: [GraphRAG面试题, LightRAG面试题, RAG面试题, 大模型面试题, LLM面试, 知识图谱RAG, GraphRAG原理, LightRAG原理, 微软GraphRAG, RAG优化, 大厂面试, AI面试]
tags: [GraphRAG面经, LightRAG面经, RAG面试, 大模型面试, LLM面试, AI求职]
---

# GraphRAG与LightRAG大厂面试题汇总：从RAG到知识图谱检索

之前写了[讲透RAG](./rag_interview.md)，把向量检索、混合检索、Rerank、幻觉处理这些讲透了。
...
先避开一个最近很容易混淆的词：这里的 GraphRAG 说的是**知识图谱检索**，不是 Agent 的控制图。后者讲“任务下一步怎么走”，详见 [Graph Engineering 与 Agent 图编排](./graph_engineering_interview.md)；本篇讲“知识里的实体关系怎么检索”。
...
1. [RAG 检索到了但答不对——传统 RAG 的三个天花板](#1-rag-检索到了但答不对传统-rag-的三个天花板)
2. [RAG 的演进：从"找文本"到"找关系"](#2-rag-的演进从找文本到找关系)
3. [GraphRAG 的完整链路：从原始文档到社区摘要](#3-graphrag-的完整链路从原始文档到社区摘要)
4. [GraphRAG 落地踩的三个坑](#4-graphrag-落地踩的三个坑)
5. [LightRAG：更轻、更快、增量友好](#5-lightrag更轻更快增量友好)
6. [GraphRAG 和 LightRAG 到底怎么选](#6-graphrag-和-lightrag-到底怎么选)
...
[讲透RAG](./rag_interview.md)讲的混合检索、Rerank、Query 改写，都是在"找更好的文本块"。但有些问题需要的不是更好的文本块，是**实体之间的关系**和**全局的结构性理解**。
...
**Naive RAG**——最原始的 RAG：文档切块 → 向量化 → 检索 → 塞给 LLM 生成。问题很多：检索不准、幻觉严重、没有 Rerank。[讲透RAG](./rag_interview.md)讲的很多优化手段，在 Naive RAG 里都没有。

**Advanced RAG**——在[讲透RAG](./rag_interview.md)里详细讲过的那些优化：混合检索补上关键词匹配的短板、Rerank 做精排提准、Query 改写对付模糊问题、Parent-Child 检索兼顾精度和上下文。这些优化确实把"找文本块"这件事做到了极致。
