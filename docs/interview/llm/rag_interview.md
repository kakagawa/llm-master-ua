---
title: 2026年RAG大厂面试题汇总：向量检索、混合检索、Rerank、幻觉处理高频问题与回答思路
description: 汇总2026年RAG方向大厂高频面试题，覆盖向量检索原理、混合检索、Rerank重排序、Chunk切分、Embedding模型选型、幻觉处理、RAG优化、Agentic RAG和评测指标等核心问题，配回答思路和代码示例，适合大模型应用开发面试复习。
keywords: [RAG面试题, 大模型面试题, LLM面试, 向量检索面试, 混合检索面试, Rerank面试, RAG优化, 幻觉处理, 大模型RAG, RAG面经, 大厂面试, AI面试]
tags: [RAG面经, 大模型面试, LLM面试, AI求职]
faq:
  - q: "RAG 是什么？为什么需要 RAG？"
    a: "RAG（检索增强生成）在大模型生成回答前，先从外部知识库检索相关信息塞进 Prompt，让模型基于事实回答，主要解决大模型知识截止、私有数据无法触达和容易幻觉这三个问题。"
  - q: "纯向量检索有什么问题？为什么要用混合检索？"
    a: "纯向量检索靠语义相似度，在精确匹配、专业术语和专有名词上召回差。混合检索同时跑向量检索和关键词检索 BM25，用 RRF 合并结果取长补短，是生产环境的主流做法。"
  - q: "Rerank 是什么？为什么检索之后还要重排序？"
    a: "检索用 Bi-Encoder 快但粗，只能算大概相关；Rerank 用 Cross-Encoder 把问题和文档拼在一起精算相关性，把真正相关的排到前面。先检索捞 Top-20 再 Rerank 取 Top-5 是标配流程。"
  - q: "RAG 的幻觉怎么处理？"
    a: "常用组合是 Prompt 约束只基于检索结果回答、输出自校验、引用标注、温度调低、检索与生成结果对齐，以及相似度低于阈值时兜底回答未找到相关信息，多策略叠加比单用一种效果好。"
  - q: "RAG 和微调怎么选？"
    a: "RAG 适合知识频繁更新、需要可溯源的场景，微调适合固化领域能力或改变风格。多数应用先用 RAG，确有必要再考虑微调，两者也可以结合使用。"
---

# RAG大厂面试题汇总：向量检索、混合检索、Rerank、幻觉处理高频问题

今年[知识星球](https://programmercarl.com/other/kstar.html)里，录友反馈最多的面试变化就是：**RAG 成了必考项**。
...
1. [RAG 是什么？为什么需要 RAG？](#_1-rag-是什么为什么需要-rag)
2. [RAG 的完整链路是怎样的？](#_2-rag-的完整链路是怎样的)
3. [向量检索的原理是什么？](#_3-向量检索的原理是什么)
4. [向量数据库怎么选？Milvus、FAISS、Qdrant 各自适合什么场景？](#_4-向量数据库怎么选milvusfaissqdrant-各自适合什么场景)
5. [纯向量检索有什么问题？为什么需要混合检索？](#_5-纯向量检索有什么问题为什么需要混合检索)
6. [Rerank 是什么？为什么检索之后还要重排序？](#_6-rerank-是什么为什么检索之后还要重排序)
7. [Chunk 怎么切？切大了切小了各有什么问题？](#_7-chunk-怎么切切大了切小了各有什么问题)
8. [Embedding 模型怎么选？中文场景选什么？](#_8-embedding-模型怎么选中文场景选什么)
9. [RAG 的幻觉怎么处理？](#_9-rag-的幻觉怎么处理)
10. [RAG 检索效果不好怎么优化？](#_10-rag-检索效果不好怎么优化)
11. [Agentic RAG 是什么？和普通 RAG 有什么区别？](#_11-agentic-rag-是什么和普通-rag-有什么区别)
12. [大厂真实面试追问汇总](#_12-大厂真实面试追问汇总)
...
幻觉是 RAG 项目最大的工程挑战，面试官必问（它本质是检索召回和生成忠实度的级联问题，更深一层的拆解见 [RAG 落地最难的地方在哪](./rag_hardest_parts_interview.md)）。
...
Agentic RAG 把 [Agent](./agent_interview.md) 的规划能力引入 RAG——LLM 自己判断：需要检索哪些数据源？检索结果够不够？不够就换个角度再检索。
...
还有一类问题普通 RAG 和 Agentic RAG 都吃力：实体之间的复杂关联、多跳推理、全局关系归纳。这种场景要把检索从向量切到知识图谱，具体见 [GraphRAG 与 LightRAG 大厂面试题汇总](./graphrag_interview.md)。
...
另外面试官很爱把 RAG 和微调放一起问"知识更新到底该用哪个、什么时候才值得微调"，这块的取舍判断可以看 [SFT、RLHF、DPO 面试详解](./finetuning_sft_rlhf_interview.md)。
