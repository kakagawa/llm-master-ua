---
title: RAG落地最难的地方在哪？大厂面试题：文档预处理、召回质量、生成忠实度与面试回答思路
description: 深度拆解RAG生产落地三大难点：文档预处理为什么被低估（解析/清洗/增量更新），召回质量为什么难调（语义鸿沟/分块策略/精度vs召回率），生成忠实度为什么被忽视（幻觉/Lost in the Middle/上下文利用不充分），三个环节级联放大，附面试回答框架。
keywords: [RAG难点, RAG落地, 文档预处理, 召回质量, 生成忠实度, RAG幻觉, 分块策略, 混合检索, Rerank, 大模型面试题, RAG面试, 知识库构建, 大厂面试]
tags: [RAG面经, 大模型面试, LLM面试, AI求职, 大模型求职]
---
...
先看 RAG 的完整链路（向量检索、Rerank、Chunk 这些基础概念如果还不熟，建议先看 [RAG 大厂面试题汇总](./rag_interview.md)）：用户提问 → 检索相关文档 → 把文档和问题一起丢给大模型 → 大模型生成回答。

这个链路看似简单，实际每个环节都有坑：

![RAG链路三个难点级联放大](https://file1.kamacoder.com/i/web/20260512175435_01.png)
...
![文档解析的典型问题](https://file1.kamacoder.com/i/web/20260512175445_02.png)
...
![语义鸿沟](https://file1.kamacoder.com/i/web/20260512175449_03.png)
...
![分块策略的两难](https://file1.kamacoder.com/i/web/20260512175454_04.png)
...
![精度vs召回率的取舍](https://file1.kamacoder.com/i/web/20260512175501_05.png)
...
但如果知识本身是图状的——实体之间多跳关联、需要全局归纳，向量召回再怎么调也会碰天花板，这时候得换知识图谱检索的思路，见 [GraphRAG 与 LightRAG 大厂面试题汇总](./graphrag_interview.md)。
...
**这种幻觉最危险，因为它和正确信息混在一起，用户很难分辨。** RAG 只是幻觉的一个来源，Agent 系统里还有工具调用、多轮上下文等更多触发点，系统性的幻觉约束思路见 [Agent 系统如何约束大模型幻觉](./agent_hallucination_control_interview.md)。
...
[Transformer大厂面试题汇总](./transformer_interview.md) 里讲过 "Lost in the Middle" 现象：大模型对上下文中间位置的信息关注度最低。
