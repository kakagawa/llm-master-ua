---
title: Claude Code为什么不用RAG检索代码？Grep、Glob、Read、子Agent与代码检索设计哲学面试详解
description: 深度拆解Claude Code为什么不用传统RAG检索代码，而是用Grep、Glob、Read等工具实时搜索代码库。从代码检索和文档检索的差异、RAG在代码场景的结构破坏、索引滞后、精确匹配短板，到Agent多轮检索、子Agent隔离上下文、混合检索边界和面试回答模板，系统讲透AI编程工具的代码检索设计。
keywords: [Claude Code不用RAG, Claude Code Grep, Claude Code代码检索, AI编程面试, Agent代码检索, RAG检索代码, 向量检索代码, Grep和RAG区别, Glob Read Grep, 子Agent, Task工具, 上下文管理, 代码库检索, AI Coding, 大模型面试题, Agent面试题, Claude Code面试]
tags: [Claude Code面经, AI编程面试, Agent面经, RAG面试, 大模型面试, LLM面试, AI求职]
---
...
之前写 [RAG大厂面试题汇总](./rag_interview.md) 的时候，卡哥也讲过，RAG 是大模型应用开发绕不开的基础能力。
...
1. 代码检索到底在解决什么问题
2. 为什么传统 RAG 放到代码里会别扭
3. Claude Code 的 Grep、Glob、Read 为什么不是简单 shell 命令
4. 多轮 Agent 检索为什么比一次性 Top-K 更适合写代码
5. 子 Agent 为什么能解决上下文污染
6. grep 也不是银弹，什么场景仍然该用 RAG 或混合检索
7. 面试里怎么回答，才能答出架构感
...
1. 先说结论：这不是 grep 赢了 RAG
2. 代码检索和文档检索，根本不是一类问题
3. RAG 检索代码的四个硬伤
4. Claude Code 为什么把检索拆成三件套
5. Grep 不是简单 grep，而是受控的代码搜索工具
6. Glob 和 Read 解决的是"文件入口"和"上下文边界"
7. 真正关键的是多轮循环，不是某个工具
8. 子 Agent：把探索过程隔离出去
9. grep 方案也有边界，什么时候 RAG 仍然有价值
10. 面试怎么答
...
之前写 [Claude Code大厂面试题汇总](./claude_code_deep_dive.md) 的时候讲过，Claude Code 的底层形态就是多轮循环：
