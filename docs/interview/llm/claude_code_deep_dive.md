---
title: Claude Code大厂面试题汇总：源码泄露、Agent Loop、系统提示词、工具链、上下文管理、安全机制全拆解 | 万字深度解析工作原理
description: 从源码泄露事件出发，深度拆解Claude Code的核心架构：while循环Agent模式、8700 token系统提示词、18+内置工具、子Agent机制、200K上下文窗口管理、23层安全检查、CLAUDE.md记忆系统、双模型策略。每章附面试官追问方向，彻底搞懂AI编程助手的底层原理。
keywords: [Claude Code面试题, Claude Code原理, Agent Loop, AI编程助手, 系统提示词, Function Calling, 子Agent, 上下文管理, CLAUDE.md, AI Coding面试, Anthropic, Claude, AI编程工具原理, Cursor原理, Windsurf原理, AI IDE, 大模型面试, AI编程面试]
tags: [Claude Code面经, AI编程面试, 大模型面试, Agent架构, 深度解析]
faq:
  - q: "Claude Code 的底层架构是什么？和普通对话有什么区别？"
    a: "Claude Code 是 while 循环的 Agent 模式，模型决定调用工具、执行后把结果喂回模型、继续循环直到任务完成。普通对话是一问一答，Claude Code 能多轮自主调用工具完成复杂编程任务。"
  - q: "为什么 Claude Code 用专用工具而不是直接用 Bash？"
    a: "专用工具如 Read、Edit、Glob、Grep 有明确的输入输出 schema，模型更容易正确调用，也方便做权限控制和安全校验；全靠 Bash 则难以约束行为、容易出错和越权。"
  - q: "Claude Code 怎么管理 200K 上下文窗口？"
    a: "通过 Auto-Compact 自动压缩历史对话、保留任务关键状态、用子 Agent 隔离上下文等机制，避免长任务把上下文窗口撑爆。"
  - q: "Claude Code 的子 Agent 机制怎么工作？"
    a: "主 Agent 通过 Agent 工具启动 Explore、Plan 等子 Agent，各自独立执行后把结果回传汇总。适合前期调研、大范围搜索这类消耗大量上下文的子任务，避免污染主 Agent。"
  - q: "CLAUDE.md 怎么写才有效？"
    a: "三条原则是写约束不写愿望、写具体不写抽象、写原因不写指令。禁止性规则比建议性规则更被遵守，明确的判断标准和原因能让模型举一反三。"
  - q: "学 Claude Code 原理对用其他 AI 编程工具有用吗？"
    a: "有用。Cursor、Windsurf、Copilot 等工具底层架构大同小异，都是 Agent Loop 加工具链加上下文管理加系统提示词，理解 Claude Code 就能理解这一类 AI 编程工具。"
---
...
1. [Claude Code源码是怎么泄露的？泄露了什么？](#一claude-code源码是怎么泄露的泄露了什么)
2. [AI编程工具的底层架构是什么？和普通对话有什么区别？](#二ai编程工具的底层架构是什么和普通对话有什么区别)
3. [8700 token的系统提示词里写了什么？](#三8700-token的系统提示词里写了什么)
4. [18+内置工具怎么设计？为什么要专用工具不用Bash？](#四18内置工具怎么设计为什么要专用工具不用bash)
5. [子Agent机制怎么工作？什么场景会启动子Agent？](#五子agent机制怎么工作什么场景会启动子agent)
6. [200K上下文窗口怎么管理？压缩机制是什么？](#六200k上下文窗口怎么管理压缩机制是什么)
7. [23层安全检查怎么防护？权限怎么评估？](#七23层安全检查怎么防护权限怎么评估)
8. [CLAUDE.md和记忆系统怎么让AI「认识」项目？](#八claudemd和记忆系统怎么让ai认识项目)
9. [双模型策略怎么分工？成本怎么控制？](#九双模型策略怎么分工成本怎么控制)
10. [从Claude Code能学到什么对开发者有用的？](#十从claude-code能学到什么对开发者有用的)
11. [大厂真实面试追问汇总](#十一大厂真实面试追问汇总)
...
1. **默认2分钟超时**——防止无限循环
2. **不支持交互式命令**——不能用vim、不能用git rebase -i
...
而所谓"逐步加规则"，加的就是循环这个控制结构上的工程治理——上下文、状态、预算、工具、终止判断，这套东西现在有个专门的名字叫 Loop Engineering，系统讲解见 [从 ReAct 到 Loop Engineering：Agent 到底在循环什么](./loop_engineering_interview.md)。
...
1. **分层嵌入**——安全规则不能只放在系统提示词里，要嵌进工具描述、专用规则、用户Hooks，做到即使模型"忘记"某一层，还有其他层兜底
...
1. **专用工具节省Token**——Edit只发diff，Glob/Grep按需搜索，不全量加载文件
2. **分层压缩**——早期对话摘要压缩，中间结果只保留关键信息，系统提示词按模块裁剪
3. **子Agent隔离**——探索代码库交给Explore Agent，内部消耗100K+，返回父Agent只占1,500-2,000 token
4. **双模型路由**——简单任务用小模型，减少单次调用Token消耗
...
- **Cursor**：IDE深度集成，代码补全是独立功能（不经过Agent Loop），Agent模式类似Claude Code
- **Windsurf**：Cascade模式自动多步执行，强调"意图理解"
- **Claude Code**：纯CLI，权限控制最严格，子Agent机制最完善
...
1. **写约束不写愿望**——"不要加注释"比"写简洁的代码"更有用，模型对禁止性规则的遵守远强于建议性规则
2. **写具体不写抽象**——"函数名用驼峰"比"遵循项目风格"更有用，模型需要明确的判断标准
3. **写原因不写指令**——"VuePress 1.x插件版本必须统一为1.5.3，混用会导致运行时错误"比"注意插件版本"更有用，知道原因模型才能举一反三

**Q：实际使用AI编程工具遇到过什么问题？**

常见五个坑：

1. **上下文污染**——对话太长后AI开始"忘记"早期约定 → 解法：新任务开新对话，关键指令放最近的消息
2. **幻觉编辑**——AI编辑了一个不存在的文件路径 → 解法：用Glob确认路径再编辑
3. **过度重构**——AI把简单任务改成复杂设计 → 解法：CLAUDE.md里明确写"不要引入不必要的抽象"
4. **权限绕过**——用户习惯性点"允许全部" → 解法：用plan模式先让AI分析，确认方案再切回default模式
5. **成本爆炸**——复杂任务消耗大量Token → 解法：拆分子任务，用Explore Agent做前期调研减少主Agent消耗
...
这篇是整体拆解，几个模块还有专门的深挖：它为什么用 Grep 而不是 RAG 检索代码看 [Claude Code 为什么不用 RAG 检索代码](./claude_code_grep_rag_interview.md)，200K 上下文窗口怎么压缩和管理看 [Claude Code 上下文窗口面试详解](./claude_code_context_window_interview.md)，它和 Hermes Agent、OpenClaw 的架构对比看 [Agent 框架横评](./agent_framework_comparison.md)。想从"程序员怎么用好 AI 编程"这个角度看，可以读 [Vibe Coding 大厂面试题汇总](./vibe_coding_interview.md)。
