---
title: Agent上下文漂移与工具调用幻觉大厂面试题：根因分析、检测信号、分层解法与面试回答思路
description: 深度拆解Agent上下文漂移和工具调用幻觉两类可靠性问题，从注意力机制、概率生成、上下文污染、工具Schema约束等角度分析根因，整理检测信号、分层解法、输出校验、工具调用兜底策略和大厂面试回答框架，适合准备Agent工程化、LLM应用开发和AI可靠性面试。
keywords: [Agent上下文漂移, 工具调用幻觉, Agent可靠性, Agent幻觉, 大模型面试题, Agent面试, Function Calling幻觉, 上下文管理, Agent工程化, 大模型应用开发, 约束解码, 注意力稀释, 大厂面试]
...
[Transformer大厂面试题汇总](./transformer_interview.md) 里讲过 Self-Attention 的核心机制：每个 Token 对所有 Token 计算注意力权重，加权求和得到表示。这个机制带来了两个直接后果：
