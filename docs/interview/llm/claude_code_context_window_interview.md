---
title: Claude Code上下文窗口面试详解：Auto-Compact、上下文压缩、工具结果裁剪与Agent记忆管理
description: 从上下文窗口的本质讲起，循序渐进拆解Claude Code为什么会在Agent循环中快速消耗上下文，业界常见上下文管理方案的边界，以及Auto-Compact如何通过工具结果裁剪、会话摘要、子Agent隔离和记忆重注入完成上下文压缩与任务接续，适合大模型应用开发和AI编程面试复习。
keywords: [Claude Code上下文窗口, Claude Code Auto-Compact, Claude Code压缩, 上下文压缩, Context Window, Context Engineering, Agent上下文管理, AI编程面试, Claude Code面试题, 大模型上下文窗口, 工具调用, 子Agent, CLAUDE.md, 大模型面试, LLM Agent]
tags: [Claude Code面经, AI编程面试, 大模型面试, Context Engineering, Agent上下文管理]
---
...
- [Claude Code大厂面试题汇总](./claude_code_deep_dive.md)：从 Agent Loop、工具链、系统提示词、安全机制整体拆了一遍
- [Claude Code为什么不用RAG检索代码](./claude_code_grep_rag_interview.md)：重点讲 Grep、Glob、Read、子 Agent 和代码检索的关系
...
1. [面试官到底想考什么？](#一面试官到底想考什么)
2. [上下文窗口不是记忆，只是本轮输入空间](#二上下文窗口不是记忆只是本轮输入空间)
3. [为什么 Agent 跑起来分分钟爆窗口？](#三为什么-agent-跑起来分分钟爆窗口)
4. [上下文满了，真正的问题不是放不下，而是质量下降](#四上下文满了真正的问题不是放不下而是质量下降)
5. [业界常见上下文方案，为什么单独用都不够看？](#五业界常见上下文方案为什么单独用都不够看)
6. [Claude Code 的上下文治理，不是只靠 Auto-Compact](#六claude-code-的上下文治理不是只靠-auto-compact)
7. [Auto-Compact 什么时候触发？](#七auto-compact-什么时候触发)
8. [Auto-Compact 压什么、留什么、丢什么？](#八auto-compact-压什么留什么丢什么)
9. [摘要 prompt 不是"总结一下"，而是生成任务状态快照](#九摘要-prompt-不是总结一下而是生成任务状态快照)
10. [压完之后，Claude Code 怎么接续对话？](#十压完之后claude-code-怎么接续对话)
11. [这道面试题该怎么答？](#十一这道面试题该怎么答)
12. [面试官可能继续追问什么？](#十二面试官可能继续追问什么)
13. [普通开发者怎么用得更稳？](#十三普通开发者怎么用得更稳)
...
1. 先解释上下文窗口是什么
2. 再解释 Agent 为什么比普通聊天更容易爆窗口
3. 再说常见方案为什么单独用都不够
4. 最后讲 Claude Code 是分层治理，Auto-Compact 只是最后一层
