---
title: 2026最全大模型学习路线：从零入门到上手项目，该按什么顺序学
description: 程序员怎么学大模型？这是一条给后端开发者的大模型学习路线。Java、C++、Go转大模型应用开发，不用从公式推导学起。本文按入门认知、Prompt调用、RAG检索增强、Agent智能体、微调选型、部署压测、Transformer原理七个阶段，讲清每一步学什么、为什么按这个顺序学、哪些该深挖哪些点到为止，附每个阶段的配套文章和大模型面试准备建议。
keywords: ["大模型学习路线", "大模型怎么学", "大模型入门", "大模型从零开始", "程序员转大模型", "后端转大模型", "大模型应用开发学习路线", "大模型自学", "大模型学习路径", "大模型需要学什么", "RAG", "Agent", "Transformer", "大模型面试"]
tags: ["大模型", "应用开发", "学习路线"]
---
...
如果你还没搞清楚自己要走哪条路，建议先看这两篇打底：[大模型应用开发、算法岗、开发岗到底什么区别](./application_development.md) 和 [大模型应用开发到底在做什么](./app_dev_overview.md)。想清楚定位，再开始走路线，不然容易学偏。
...
建议从这篇开始，它把核心概念串成一条进化线：[大模型关键词全解：从 Prompt 到 Agent 到 MCP，一篇搞懂 13 个核心概念](./llm_keywords.md)。

然后补几块认知拼图：

- [大模型到底是怎么训练出来的](./how_llm_trained.md)——知道它是怎么来的，才理解它为什么会幻觉
- [大模型 API 到底怎么计费](./llm_pricing.md)——这是你以后天天要算的账
- [大模型蒸馏到底是什么](./model_distillation.md)——理解小模型、降本是怎么回事
...
- [结构化输出：JSON Schema 怎么约束](../app/structured_output.md)——业务系统离不开稳定的结构化输出
- [同步、异步、流式输出怎么选](../app/streaming_output.md)
- [Function Calling 详解：大模型怎么调用工具](../app/function_calling.md)——这是后面 Agent 的地基
...
- [为什么有了大模型还需要 RAG](../app/why_rag.md)
- [RAG 完整链路拆解：离线阶段和在线阶段](../app/chain_of_rag.md)
- [Embedding 是什么：语义压缩与模型选型](../app/embedding.md)
- [向量数据库解决了什么问题](../app/vector_database.md)
- [RAG 切片策略：四种方式对比](../app/how_to_chunking.md)
- [RAG 系统答不准的常见问题排查](../app/rag_problems.md)
- [RAG 优化思路：Query 改写到 Context 压缩](../app/rag_optimization.md)
...
如果你就是冲着 Agent 来的，这一阶段我单独拆了一篇更细的 [AI Agent 学习路线](../app/agent_learning_roadmap.md)，把下面这些文章该按什么顺序学讲透了，建议照那篇走。

- [Agent 到底是什么？和普通大模型问答有什么区别](../app/agent_intro.md)
- [ReAct、Reflection、规划执行：三种常见思路怎么选](../app/react_reflection_planning.md)
- [Agent vs Workflow：什么时候根本不需要 Agent](../app/agent_vs_workflow.md)——这一篇能帮你少踩很多"过度 Agent 化"的坑
- [工具设计决定 Agent 上限](../app/agent_tool_design.md)
- [MCP 协议详解：和 Function Calling 有什么区别](../app/mcp_protocol.md)
- [Agent 为什么容易翻车](../app/agent_failure_modes.md)
- [Agent 的记忆：短期、长期、RAG 到底什么关系](../app/agent_memory.md)
- [Agent 怎么评估：任务完成率与可靠性度量](../app/agent_evaluation.md)
...
- [SFT、RLHF、DPO：微调方法全景认知](../app/finetuning_sft_rlhf_dpo.md)
- [LoRA / QLoRA：为什么低秩微调这么流行](../app/lora_qlora.md)
...
- [为什么都绕不开 Transformer](../transformer/transformer_base_1.md)
- [Attention 机制：Q、K、V 是什么](../transformer/qkv.md)
- [Multi-Head Attention：为什么一个头不够](../transformer/mha.md)
- [位置编码：Transformer 为什么必须知道顺序](../transformer/pos_encode.md)

再手撕代码，面试有底气：

- [手撕 Attention：不依赖框架从零实现](../transformer/attention_code.md)
- [手撕 Tiny Transformer：从零拼出完整模型](../transformer/tiny_transformer_code.md)

完整的 Transformer 原理篇和手撕篇，都在[卡码大模型专栏首页](../)按章节列好了，照着顺序走即可。
...
大模型相关的面试题，我整理在了[大模型面经](../../interview/llm/)里；简历怎么写，可以看[简历专栏](../../jianli/)。学和找工作不要分两段，**边学边按四要素把项目写进简历**，效率最高。

我也在[知识星球](https://mp.weixin.qq.com/s/lLq0U6momirYMwzqyQ5KXg)里辅导过很多录友转大模型，路线、项目、面试一路盯着改，少走弯路。
