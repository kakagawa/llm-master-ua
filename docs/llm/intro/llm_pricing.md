---
title: 大模型 API 到底怎么计费？一个汉字几个 token？GLM-5.1、GPT-5.4、Opus 4.7 算给你看
description: 大模型API计费看不懂？输入输出价格为什么不一样？长上下文为什么更贵？缓存命中能省多少钱？一个汉字到底占几个token？用GLM-5.1、GPT-5.4、Opus 4.7的实际定价，把大模型计费从token到账单全算清楚，再告诉你三个模型到底该选谁。
keywords: [大模型计费, LLM API价格, token计费, GLM-5.1价格, GPT-5.4价格, Opus 4.7价格, 大模型token, 中文token, API调用费用, 缓存命中, 上下文长度, 大模型成本, Prompt Cache, 输入输出token, 大模型选型]
tags: [大模型入门, LLM计费, API调用, 大模型选型]
---
...
1. [Token 是什么？一个汉字几个 token？](#1-token-是什么一个汉字几个-token)
2. [GLM-5.1 的定价结构拆解](#2-glm-51-的定价结构拆解)
3. [输入和输出为什么价格不一样？](#3-输入和输出为什么价格不一样)
4. [上下文长度为什么影响价格？](#4-上下文长度为什么影响价格)
5. [缓存是什么？能省多少钱？](#5-缓存是什么能省多少钱)
6. [完整算一遍：从请求到账单](#6-完整算一遍从请求到账单)
7. [GLM-5.1 vs GPT-5.4 vs Opus 4.7 价格对比](#7-glm-51-vs-gpt-54-vs-opus-47-价格对比)
8. [三个模型的隐性成本，标价里看不到](#8-三个模型的隐性成本标价里看不到)
9. [真实场景算一遍：到底谁便宜？](#9-真实场景算一遍到底谁便宜)
10. [到底怎么选？](#10-怎么选给你一个决策公式)
...
[GLM-5.1](https://www.bigmodel.cn/invite?icode=h5645fTH07PKdN0cj%2FTnKmczbXFgPRGIalpycrEwJ28%3D) 的输出价格是输入的 **4 倍**。这不是智谱故意坑你，是输出确实比输入费算力。
...
缓存是 [GLM-5.1](https://www.bigmodel.cn/invite?icode=h5645fTH07PKdN0cj%2FTnKmczbXFgPRGIalpycrEwJ28%3D) 定价表里最容易让人懵的部分。什么叫"缓存存储"？什么叫"缓存命中"？
...
你问 [GLM-5.1](https://www.bigmodel.cn/invite?icode=h5645fTH07PKdN0cj%2FTnKmczbXFgPRGIalpycrEwJ28%3D)："用 Python 写一个快排"，模型输出了 300 字代码。
...
前面用 [GLM-5.1](https://www.bigmodel.cn/invite?icode=h5645fTH07PKdN0cj%2FTnKmczbXFgPRGIalpycrEwJ28%3D) 讲了计费原理，现在把三大主流模型放一起比。为了直观，统一换算成美元（¥1 ≈ $0.14）。
...
**[GLM](https://www.bigmodel.cn/invite?icode=h5645fTH07PKdN0cj%2FTnKmczbXFgPRGIalpycrEwJ28%3D) 依然最便宜，GPT 紧随其后，Opus 是 GLM 的 5 倍。**
...
**短对话、简单问答、分类任务** → **[GLM-5.1](https://www.bigmodel.cn/invite?icode=h5645fTH07PKdN0cj%2FTnKmczbXFgPRGIalpycrEwJ28%3D)**
...
**GPT-5.4 看起来便宜，Opus 4.7 用起来稳定，[GLM-5.1](https://www.bigmodel.cn/invite?icode=h5645fTH07PKdN0cj%2FTnKmczbXFgPRGIalpycrEwJ28%3D) 算起来最省。** 别被标价骗了，真正成本要看场景、看成功率、看隐性费用。
...
同样的任务走 [GLM-5.1](https://www.bigmodel.cn/invite?icode=h5645fTH07PKdN0cj%2FTnKmczbXFgPRGIalpycrEwJ28%3D)：
