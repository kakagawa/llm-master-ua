---
title: 大模型关键词全解：从Prompt到Agent到MCP，一篇搞懂13个核心概念
description: 大模型相关术语一篇讲透：Prompt、Token、上下文窗口、幻觉、Structured Output、Function Calling、RAG、Embedding、向量数据库、微调、Agent、MCP、Skill，从最简单的聊天开始，每个概念都是上一个的自然延伸。
keywords: ["大模型关键词", "Prompt", "Agent", "RAG", "MCP", "Skill", "Function Calling", "上下文窗口", "Token", "幻觉", "Embedding", "向量数据库", "微调", "大模型入门"]
tags: ["大模型", "应用开发"]
---
...
所以实际工程里，不是拼命塞满上下文，而是**精准控制上下文里放什么**。关于从 Prompt Engineering 到 Context Engineering 再到 Harness Engineering 的演进，可以看[这篇 Harness Engineering 面试文章](https://mp.weixin.qq.com/s/yP3Clk7EdzdmzV4ImrAxug)。
...
关于 RAG 的面试高频考点（链路、向量检索、混合检索），在[RAG 面试文章](https://mp.weixin.qq.com/s/74EC_fteGzIPS46ohX_ncw)里有完整整理。
...
在[Embedding 详解](https://mp.weixin.qq.com/s/huCc9mEeeIvMdAKMIH5ITg)里我们讲过，Embedding 不是在生成文本，而是在压缩语义。
...
如果传统 RAG 的向量检索已经满足不了你，可以看看[GraphRAG 面试文章](https://mp.weixin.qq.com/s/RJWR1tR9jYXh-KbC-7I-7w)，了解 RAG 从"找文本"到"找关系"的演进。
...
想更深入了解 Agent 的工作模式（ReAct、Plan-and-Execute 等）和 Agent vs Workflow 的区别，可以看[Agent 面试文章](https://mp.weixin.qq.com/s/eE8PHFRXyTirRMrlJDyZGg)。
...
MCP 解决的是**规模化的问题**——当你只有一两个工具，手写对接也行；但当工具越来越多、模型经常换，标准化协议就是刚需。Claude Code 就是 MCP 的典型实践，它的 18+ 内置工具就是通过标准化协议对接的，具体可以看[Claude Code 深度拆解](https://mp.weixin.qq.com/s/ru-g8h5WgBc2bE-v3gVg7w)。
