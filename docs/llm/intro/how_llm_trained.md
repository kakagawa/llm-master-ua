---
title: 大模型到底是怎么训练出来的？预训练、模型参数、SFT微调、RLHF对齐、推理一次讲明白
description: 大模型到底是怎么训练出来的？本文用程序员能听懂的方式，拆解大模型从数据清洗、Token化、Transformer预训练、模型参数更新、监督微调SFT、RLHF/DPO偏好对齐，到推理阶段逐Token生成答案的完整流程，讲清楚ChatGPT类大模型为什么能聊天、为什么会幻觉，以及大模型训练和大模型应用开发之间的关系。
keywords: [大模型训练, LLM训练流程, 模型参数, 大模型参数, 预训练, 大模型预训练, Token, Tokenizer, Transformer, SFT, 监督微调, RLHF, DPO, 偏好对齐, 大模型推理, inference, KV Cache, temperature, top-p, 大模型幻觉, ChatGPT原理, 大模型应用开发, AI面试]
tags: [大模型入门, 大模型训练, 预训练, SFT微调, 大模型推理]
---
