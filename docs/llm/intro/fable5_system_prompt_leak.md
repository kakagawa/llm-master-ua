---
title: "Claude Fable 5被破解了？12万字系统提示词全曝光：你还没开口，3万token就没了"
description: "2026年6月，研究者Pliny the Liberator公布了疑似Claude Fable 5的系统提示词，约12万字符、3万token、1585行、72个段落。本文借这次泄露，讲清楚系统提示词到底是什么、3万token的预算都花在哪、用户还没开口就先吃掉一口上下文窗口意味着什么、为什么大厂敢这么写（Prompt Cache兜底），以及普通开发者能从中抄到什么Context Engineering的作业，适合大模型应用开发、AI编程和Agent开发学习。"
keywords: [Claude Fable 5, Fable 5系统提示词, 系统提示词泄露, system prompt leak, Pliny the Liberator, 12万字提示词, 3万token, Context Engineering, 上下文工程, Prompt Cache, 上下文窗口, token成本, 工具定义, prompt injection, prompt extraction, 越狱, jailbreak, 大模型应用开发, AI编程, Agent开发, CLAUDE.md]
tags: [Claude, 系统提示词, Context Engineering, Prompt Cache, 大模型成本优化, AI编程]
---

# Claude Fable 5被破解了？12万字系统提示词全曝光

> 不少读者问我，如何才能用上Claude pro（opus 4.8，Fable），我在这里篇单独讲一下：[国内Claude充值会员的方法](https://notes.kamacoder.com/qita/0002.claudepay.html)

录友们好。

Anthropic 刚发了 [Claude Fable 5](../news/claude-fable-5.md)，号称目前最强的公开模型。
...
因为就算打了这三个折扣，这份提示词的**结构**依然极有参考价值。
...
什么该拒答、什么是危险请求、怎么对待用户的情绪。这里面有个细节特别说明问题：提示词里曾经有个**已经失效的求助热线号码**，后来出问题了，于是又补了一条规则去修。你看，这玩意是一条一条"打补丁"打出来的。
...
其中上下文窗口这笔账，我们在之前聊 [上下文窗口](../../interview/llm/claude_code_context_window_interview.md) 的时候也强调过：**窗口是预算，不是仓库。** 任何固定占用，都是从你和模型的有效对话空间里扣的。
...
这也是为什么 Claude Code 这类产品，会把 Prompt Cache 当成核心架构约束——这部分我专门写过一篇 [Claude Code 为什么快](./claude_prompt_cache.md)，里面讲了前缀匹配、工具定义稳定性这些细节，感兴趣的录友可以接着看。
...
**写 prompt、写 [CLAUDE.md](./claude_md.md)，要追求的是"稳定的结构"，不是"华丽的长度"。**
...
不是单点硬刚安全分类器，而是**多个手法配合**——字符级混淆、把请求拆解成无害的碎片、再利用模型超长上下文窗口的特性，绕过防线，一点点把系统提示词诱导出来。
