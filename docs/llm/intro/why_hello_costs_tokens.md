---
title: "用上Claude之后，只发了一句「你好」，为什么消耗了十万 token？"
description: "很多录友刚用 Claude、ChatGPT 这类 AI 编程工具，发现自己只发了一句「你好」，后台却显示消耗了几万甚至十几万 token，输入、缓存读、缓存创建一堆名词看不懂，以为被坑了。本文用一组真实的后台数据，拆解一次请求里你看不见的那一大坨上下文到底是什么：系统提示词、工具定义、项目规则文件 CLAUDE.md、记忆和历史对话，再讲清输入 token、缓存创建、缓存读这三个数字分别是什么、为什么缓存读便宜五到十倍，最后告诉你这钱花得冤不冤、怎么省。适合 Claude Code、大模型 API、AI 编程和大模型应用开发入门的录友。"
keywords: [token消耗, 一句话消耗大量token, 为什么发一句话费这么多token, Claude token, Claude Code token消耗, 大模型token计费, prompt cache, 缓存读, 缓存创建, cache read, cache write, 系统提示词, system prompt, 工具定义, 上下文窗口, CLAUDE.md, 大模型成本, API费用, 大模型应用开发, AI编程, LLM token, 上下文]
tags: [大模型入门, token计费, Prompt Cache, AI编程]
---

# 用上Claude之后，只发了一句「你好」，为什么消耗了十万 token？一篇讲清你的 token 到底花哪了

最近这[两天 Claude 疯癫](../news/claude-crazy-0628.md)，开始大封号。
...
之前在聊 fable5时候，也聊过 [Claude Fable 5被破解了？12万字系统提示词全曝光](https://notes.kamacoder.com/llm/intro/fable5_system_prompt_leak.html) 
...
这文件写得越详细，越「贴合你的项目」，但也越占 token。它是个双刃剑，后面讲怎么平衡。怎么把它写得管用又不臃肿，我们专门写过一篇：[CLAUDE.md 到底怎么写](./claude_md.md)。
...
> Prompt Cache 缓存的不是你的答案，而是模型处理上下文时的中间计算。这背后还有一整套工程设计——为什么系统提示词不能随便改、工具不能随便加，都是为了不破坏缓存。想深挖的录友看这篇：[Claude Code 为什么快？Prompt Cache 一篇讲明白](./claude_prompt_cache.md)。
...
会话太长，历史对话那层会越堆越厚。Claude Code 有上下文自动压缩，必要时也可以主动开新会话——这是另一个权衡，具体怎么管理上下文，可以看[Claude上线文管理](./claude_prompt_cache.md)。
