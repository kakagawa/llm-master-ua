---
title: "Claude Code 为什么快？Prompt Cache、Plan Mode、MCP工具加载和上下文压缩一篇讲明白"
description: "基于 Anthropic 官方博客 Lessons from building Claude Code: Prompt caching is everything，本文用中文拆解 Claude Code 为什么把 Prompt Cache 当成 Agent 产品的核心架构约束，讲清前缀匹配、系统提示词分层、工具定义稳定性、Plan Mode、MCP 工具延迟加载、模型切换、上下文压缩和缓存命中率监控，适合 AI 编程、Agent 开发和大模型应用开发学习。"
keywords: [Claude Code, Prompt Cache, Prompt Caching, Claude Code缓存, Claude Code性能优化, Plan Mode, MCP工具, MCP, Tool Search, defer_loading, 上下文压缩, compact, 大模型缓存, 大模型成本优化, 大模型延迟优化, Agent架构, AI编程, AI Coding, CLAUDE.md, 大模型应用开发]
tags: [Claude Code, Prompt Cache, AI编程, Agent架构, 大模型成本优化]
---
...
[CLAUDE.md到底怎么写](./claude_md.md)，讲的是项目规则、团队规范、上下文管理怎么沉淀。

[Claude Code怎么读懂大代码库](./claude_code_large_codebase.md)，讲的是大代码库里 Claude Code 怎么靠 Agentic Search、Hooks、Skills、MCP、LSP 这些工程支架跑起来。
