---
title: 位置编码详解：Transformer为什么必须知道Token顺序，正弦编码原理
description: Transformer位置编码详解，讲清自注意力机制为什么天然不感知Token顺序，没有位置编码模型为什么分不出词序，正弦位置编码如何用不同频率表达相对和绝对位置信息，并对比可学习位置编码、RoPE等思路，整理大模型架构面试中的常见追问和回答方法。
keywords: ["位置编码", "Positional Encoding", "Transformer", "正弦编码", "Token顺序"]
