---
title: Transformer数据流动全解析：从输入文本到输出Token每一步做什么
description: Transformer数据流动全解析，先不看复杂公式，按输入文本、Token Embedding、位置编码、Attention、FFN、层堆叠、输出Token的顺序讲清每一步做什么，帮助初学者建立大模型前向传播的整体理解，也适合Transformer面试复习。
keywords: ["Transformer数据流", "Embedding", "Attention", "FFN", "大模型推理流程"]
tags: ["Transformer", "数据流"]
---
...
step4.1 升维（假设我们要升成2048维，则乘一个$512*2048$维的向量，得到$7 \times 2048$的矩阵）  
step4.2 经过激活函数  
step4.3 降维（乘一个$2048\times512$维的向量，降回原矩阵大小，$7\times512$）  
step4.4 残差连接  
经过若干层堆叠，最后输出的矩阵仍然是$7 \times 512$维
