---
title: Agent vs Workflow：什么时候根本不需要Agent，别把简单流程做复杂
description: Agent和Workflow到底有什么区别？什么时候用固定工作流，什么时候才需要Agent动态决策？本文从固定流程、开放决策、过度Agent化、混合架构和面试回答框架讲清楚Agent vs Workflow的选型边界，适合大模型应用开发和Agent面试复习。
keywords: ["Agent vs Workflow", "Agent和Workflow区别", "AI Agent", "大模型Agent", "Workflow工作流", "Agent选型", "过度Agent化", "Agent面试", "大模型应用开发", "大模型工程"]
tags: ["大模型应用", "AI Agent", "Workflow", "大模型面试"]
---
...
第一篇讲 [Agent 到底是什么](./agent_intro.md)，核心是：**Agent 不是更会聊天，而是能围绕目标持续行动。**

第二篇讲 [ReAct、Reflection、规划执行](./react_reflection_planning.md)，核心是：**Agent 要能边做边判断，做完还要检查。**
