---
title: RAG系统答不准的常见问题：检索侧五类、生成侧四类问题逐一排查
description: RAG系统答不准怎么排查？本文按检索侧和生成侧拆解常见问题，覆盖文档质量差、Chunk粒度不合理、Embedding选型不当、召回不足、缺少Rerank、Prompt拼接错误、上下文干扰、引用约束不足和模型能力边界，给出系统诊断方法和面试回答思路。
keywords: ["RAG问题排查", "RAG效果差", "检索召回不足", "Rerank", "RAG诊断", "Chunk粒度", "Context干扰", "RAG优化"]
tags: ["RAG", "大模型应用"]
---
