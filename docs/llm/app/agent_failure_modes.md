---
title: Agent为什么容易翻车？死循环、误调用、上下文污染、权限越界怎么兜底
description: Agent为什么容易翻车？本文从死循环、工具误调用、多步任务中断、上下文污染、权限越界、错误恢复和工程兜底机制讲清楚Agent系统为什么不稳定，以及如何通过步数限制、状态机、工具边界、人工确认、审计和评估让Agent更可靠，适合大模型应用开发和Agent面试复习。
keywords: ["Agent为什么容易翻车", "AI Agent翻车", "Agent死循环", "Agent误调用工具", "Agent上下文污染", "Agent权限越界", "Agent错误恢复", "Agent兜底机制", "AI Agent可靠性", "Agent面试", "大模型应用开发"]
tags: ["大模型应用", "AI Agent", "Agent可靠性", "大模型面试"]
---
...
[Agent 到底是什么](./agent_intro.md)，讲的是 Agent 和普通问答的区别。

[Agent vs Workflow](./agent_vs_workflow.md)，讲的是不要什么场景都硬上 Agent。

[工具设计决定 Agent 上限](./agent_tool_design.md)，讲的是工具描述、参数、返回值和权限边界。

[MCP 协议](./mcp_protocol.md)，讲的是工具和上下文怎么标准化接入 AI 应用。
