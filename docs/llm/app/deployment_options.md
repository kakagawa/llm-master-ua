---
title: 大模型部署方案怎么选？云API、托管推理、自部署与vLLM、SGLang选型
description: 大模型上线应该调用云API、使用托管推理，还是购买GPU自部署？本文从数据合规、模型控制、流量规模、SLO、团队运维与总拥有成本六个维度，拆解三种部署模式的责任边界，并讲清vLLM、SGLang、TensorRT-LLM、Ollama和llama.cpp不在同一选型层，给出可用于项目和面试的压测验证闭环。
keywords: [大模型部署, LLM部署, 云API, 托管推理, 私有化部署, 自部署大模型, vLLM, SGLang, TensorRT-LLM, Ollama, llama.cpp, 推理框架选型, GPU部署, 模型服务, 大模型工程化, 大模型面试]
tags: [大模型应用, 大模型部署, 推理服务, 工程化, 大模型面试]
---
...
- 数据是否安全，取决于数据边界、合同、网络和日志策略，不由“自部署”三个字自动保证；
- vLLM只是推理引擎候选之一，不能替你决定部署模式；
- 自部署省下了Token账单，却增加了GPU闲置、扩缩容、升级、监控和故障恢复成本；
- 没有相同模型、相同负载下的质量和压测数据，“更便宜”只是猜测。

真正要回答的不是“哪个工具最强”，而是：**哪些约束决定部署模式，谁负责每一层，最后用什么证据证明选择成立。**

## 简要回答

- **云API**适合先验证业务、流量波动大、团队不想维护GPU的场景，代价是模型和底层优化控制较少；
- **托管推理**允许部署指定模型或权重，但把GPU调度、服务扩缩容等工作交给平台，控制力和运维成本居中；
- **自部署**适合模型权重必须可控、数据边界严格、流量足够稳定，且团队有推理与GPU运维能力的场景；
- vLLM、SGLang、TensorRT-LLM属于自部署或托管服务里的推理引擎选择，Ollama和llama.cpp更偏本地、边缘和轻量运行，不能把它们和部署模式混成一题；
- 最终选型要用同一模型、同一硬件和真实流量压测，比较质量、SLO内Goodput与每个成功任务的总成本。
...
- 原始Prompt、RAG文档、模型输出和日志分别能到哪里；
- 供应商是否保存数据、用于训练或跨地域处理；
- 网络隔离、密钥、审计和删除策略由谁负责；
- 使用的模型权重和量化版本是否允许目标部署方式。
...
- Hugging Face已经把TGI标记为维护模式，并建议后续优先评估vLLM、SGLang、llama.cpp或MLX；
- TensorRT-LLM在2026年移除了旧TensorRT执行后端，当前官方文档以PyTorch为唯一执行后端，旧教程里的构建和转换流程不能直接照搬。
...
这和前文[《Token、成本与延迟：大模型应用的三个硬约束》](./token_cost_latency.md)是同一套逻辑：便宜但失败率高、超时多的方案，最终不一定便宜。
...
- 同一个模型版本、Tokenizer和聊天模板；
- 同一种精度或量化方案；
- 同级硬件、GPU数量和并行方式；
- 接近线上的输入长度、输出长度、并发和突发流量；
- 相同的质量测试集、超时和SLO。

然后同时比较三类证据：

- **质量**：任务成功率、结构化输出通过率、工具调用正确率和回归集；
- **性能**：TTFT、TPOT/ITL、端到端P95/P99、错误率和SLO内Goodput；
- **成本**：GPU利用率、周期总成本和每个成功任务成本。
...
[《大模型服务怎么压测？TTFT、TPOT、吞吐与Goodput》](./stress_testing.md)讲清了完整的容量验证方法。项目落地时不要照抄固定阈值，必须按业务体验设置P95/P99 SLO，并把队列时间、错误率和Goodput一起纳入。
...
本文涉及的工具现状核验于2026年7月，参考[vLLM官方文档](https://docs.vllm.ai/en/latest/)、[SGLang官方文档](https://docs.sglang.io/)、[TensorRT-LLM迁移说明](https://nvidia.github.io/TensorRT-LLM/latest/legacy/tensorrt-backend-removal.html)、[Ollama FAQ](https://docs.ollama.com/faq)、[llama.cpp官方仓库](https://github.com/ggml-org/llama.cpp)和[TGI官方仓库](https://github.com/huggingface/text-generation-inference)。
