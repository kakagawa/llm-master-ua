---
title: 大模型结构化输出：为什么自然语言不稳定，JSON Schema怎么约束
description: 大模型结构化输出为什么重要？本文讲清自然语言输出不稳定的原因，介绍JSON Schema、JSON Mode、Pydantic验证、字段约束、异常重试、输出校验和降级处理等方案，说明结构化输出适合哪些业务场景，以及面试官常问的输出稳定性问题。
keywords: ["结构化输出", "JSON Schema", "Pydantic", "大模型输出", "输出稳定性"]
tags: ["大模型调用", "结构化输出"]
---
