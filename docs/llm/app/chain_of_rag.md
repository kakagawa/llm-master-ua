---
title: RAG完整链路拆解：离线阶段和在线阶段到底做了什么
description: RAG完整链路拆解，从离线阶段的文档解析、清洗、Chunk切片、Embedding向量化、向量库入库，到在线阶段的Query理解、向量检索、混合检索、Rerank重排、Prompt拼接和模型生成，讲清每一步作用、输入输出、常见误区和大模型面试追问。
keywords: ["RAG链路", "RAG流程", "检索增强生成", "RAG离线阶段", "RAG在线阶段", "Rerank", "向量检索", "RAG面试"]
tags: ["RAG", "大模型应用"]
---
