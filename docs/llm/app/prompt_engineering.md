---
title: Prompt Engineering不是"写提示词"：结构化Prompt设计、System/User/Assistant角色与模板变量
description: Prompt Engineering到底是写提示词还是做工程？本文讲清结构化Prompt设计的思路：System/User/Assistant三种消息角色怎么分工、指令模板与变量替换怎么把业务数据注入Prompt、输出格式约束怎么让模型输出能被代码消费，以及一个可复用的五段式Prompt骨架，说明好的Prompt是工程产物而不是灵感发挥。
keywords: ["Prompt Engineering", "结构化Prompt", "提示词工程", "System Prompt", "Prompt模板", "大模型应用开发"]
tags: ["大模型应用", "Prompt工程"]
---
...
所以在Prompt层约束之外，工程上还有更硬的手段：JSON Schema校验、模型原生的结构化输出接口（`response_format`）、Pydantic做反序列化验证……这些属于"怎么保证输出一定合法"的范畴，我在[《大模型结构化输出》](./structured_output.html)那一篇里专门讲了，这里不展开。
