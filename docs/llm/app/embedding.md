---
title: Embedding是什么：语义压缩、模型选型、和Rerank的区别
description: Embedding到底是什么，为什么它是RAG和语义检索的基础设施？本文从语义压缩、向量表示、相似度计算、Embedding模型选型、中英文场景差异、向量数据库适配、和Rerank的区别等角度讲清核心概念，并整理常见误区和大模型面试追问与回答思路。
keywords: ["Embedding", "向量表示", "语义压缩", "Embedding模型选型", "Rerank", "RAG基础", "向量化", "语义检索"]
tags: ["RAG", "大模型应用"]
---
