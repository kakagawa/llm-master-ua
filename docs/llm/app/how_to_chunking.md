---
title: RAG切片策略：固定长度、递归字符、语义切分、结构感知四种方式对比
description: RAG Chunk切片策略详解，对比固定长度切分、递归字符切分、语义切分、结构感知切分四种方式，讲清Overlap参数的作用，Chunk过大或过小对召回、Rerank和生成效果的影响，以及实际大模型知识库项目中如何根据文档结构选择切片方案。
keywords: ["RAG切片", "Chunking策略", "文本切分", "递归字符切分", "语义切分", "结构感知切分", "Overlap", "RAG优化"]
tags: ["RAG", "大模型应用"]
---
