---
title: 上下文窗口有多大？Context Engineering上下文工程、滑动窗口、摘要压缩与记忆管理入门
description: 大模型上下文窗口到底有多大，输入和输出Token怎么共同占用窗口？本文从大模型应用开发和面试角度讲清Context Engineering与Prompt Engineering的区别，以及长上下文中的信息稀释、Lost in the Middle、上下文污染问题，给出检索、筛选、排序、摘要压缩、滑动窗口、外部记忆和工具结果清理等工程方案。
keywords: [上下文窗口, Context Window, Context Engineering, 上下文工程, 上下文管理, 长上下文, Token限制, Lost in the Middle, Context Rot, 滑动窗口, 上下文压缩, Compaction, 对话摘要, Agent记忆, RAG, Prompt Engineering, 大模型应用开发, LLM面试, 大模型面试题]
tags: [大模型应用, Context Engineering, 上下文管理, Prompt工程, 大模型面试]
---
...
- [OpenAI模型列表](https://developers.openai.com/api/docs/models)
- [Claude上下文窗口说明](https://platform.claude.com/docs/en/build-with-claude/context-windows)
- [Gemini长上下文说明](https://ai.google.dev/gemini-api/docs/long-context)
...
研究里有一个很典型的现象叫[Lost in the Middle](https://direct.mit.edu/tacl/article/doi/10.1162/tacl_a_00638/119630/Lost-in-the-Middle-How-Language-Models-Use-Long)：同样的关键信息，只改变它在长上下文中的位置，模型表现就可能明显变化；信息放在开头或结尾时往往更容易被利用，埋在中间时更容易丢失。
...
[《Prompt Engineering不是“写提示词”》](./prompt_engineering.md)里，我们强调了角色、任务、约束和输出格式；[《Few-shot、CoT与自我反思》](./prompt_fewshot_cot_reflection.md)又讲了怎么用示例、推理步骤和复核机制改变生成过程。它们都属于Prompt Engineering，重点是**怎么表达指令、怎么引导生成**。
...
[Anthropic关于Context Engineering的文章](https://www.anthropic.com/engineering/effective-context-engineering-for-ai-agents)给了一个很实用的定义：在模型推理时，持续策划和维护一组最优Token。换成大白话就是：**用尽可能少的高信号信息，让模型更可能做对当前任务。**
...
这才是“记忆管理”。**不是让模型永远背着全部历史，而是让信息在合适的时候重新进入工作上下文。** 关于短期记忆、长期记忆和RAG的边界，可以接着看[《Agent的记忆》](./agent_memory.md)。
...
缓存也救不了这个问题。Prompt Cache可以降低重复输入的计算或费用，但缓存里的Token通常仍占上下文窗口。**便宜地塞满，还是塞满。** 这部分的计算和缓存复用过程，可以看[《Claude Code为什么快？Prompt Cache一篇讲明白》](../claude/claude_prompt_cache.md)。
...
[RAG](./chain_of_rag.md)是上下文工程的一种重要手段，负责从外部知识库找相关材料。但Context Engineering还要管理系统规则、消息历史、工具定义、工具结果、任务状态、排序、压缩和预算。**RAG解决“找什么”，上下文工程还要解决“怎么放、放多少、什么时候更新”。**
